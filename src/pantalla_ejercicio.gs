...
class Pantalla_ejercicio:Gtk.Window
	...
	def inicio()
		...
		var hitzak="Hitz Laburra-Hitz ertaina-Hitz luzea-Hitz labur puruak"
		var sasihitzak="Sasihitz laburra-Sasihitz ertaina-Sasihitz luzea-Sasihitz labur puruak"
		
		hitzak_array=hitzak.split ("-")
		sasihitzak_array=sasihitzak.split ("-")

		box= new Box(Gtk.Orientation.VERTICAL,5)
		label1=new Label("Silabak:  ")
		label2=new Label("Hitzak:  ")
		label3=new Label("Sasihitzak:  ")
		...
		boton_crear= new Button.with_label("Crear")
		boton_salir= new Button.with_label("Salir")
		...
		hitz_kopuru= new Label("Hitzak ariketabakoitzeko"  )
		...
		if op_check2[0].active==true // hitz labur
			...
			for var x=0 to ultimo_de_lista(fonemas_elegidos)
				for var i=0 to ultimo_de_lista(datos.lemario)
					var lista_fonemas=datos.crea_lista_de_sonidos_desde_palabra(datos.lemario[i])
					for var j=0 to ultimo_de_lista(lista_fonemas)
						if lista_fonemas[j]==fonemas_elegidos[x]
			...
			if op_check1[i].active==true 
				...
				for var s=0 to ultimo_de_lista(datos.lista_silabas)
					...
					while x<=ultima(silaba)
						if fonemas_elegidos.contains(toma_cadena (silaba,x,x+2))
							estr+="x"
							x+=2
	...
	def creando_ejercicio()			
		...
		if not elegidas_opciones
			var m= new MessageDialog(this,DialogFlags.MODAL,MessageType.WARNING,ButtonsType.OK,("No se ha podido realizar ejercicio. Elija alguna opción."))
			m.show_all()
			if m.run ()==Gtk.ResponseType.OK do m.hide()
		else if lista_vacia(ejercicios)
			var m= new MessageDialog(this,DialogFlags.MODAL,MessageType.WARNING,ButtonsType.OK,("No se ha podido realizar ejercicio. Elija consonantes mas frecuentes."))
			m.show_all()
			if m.run ()==Gtk.ResponseType.OK do m.hide()
